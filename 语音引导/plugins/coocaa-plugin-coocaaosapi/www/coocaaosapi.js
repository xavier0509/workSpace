...
cordova.define("coocaaosapi", function(require, exports, module) {
    ...
    function CoocaaOSApi(){
        ...
        startapp.check("com.skyworth.lafite.srtnj.speechserver", function(message) { /* success */
            console.log("voice版本："+JSON.stringify(message));
            voiceVersion = message.versionCode
        },function (message) {
            console.log("语音不存在："+JSON.stringify(message));
            voiceVersion = 0;
        });
    ...
    CoocaaOSApi.prototype.startMovieMemberCenter = function(urltype,success,error){
        ...
        startapp.start([["action", "coocaa.intent.vip.center"],[{'business_type':urltype}]], success,error);
    }
...
//启动通用action
    ...
    CoocaaOSApi.prototype.startMyCoupon = function(sign,openId,appId,businessLine,success,error){
        ...
        startapp.start([["action", "coocaa.intent.action.MYCOUPON"],[{"sign":sign},{"openId":openId},{"appId":appId},{"businessLine":businessLine}]], success,error);
    }
