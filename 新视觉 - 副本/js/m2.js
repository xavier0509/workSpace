...
var app = {
    ...
    handleBackButton: function() {
        _czc.push(['_trackEvent', 'D20180821点击‘返回’次数', '酷开商城', TVmodel, '', '']);
        exit();
    },
    ...
    onDeviceReady: function() {
        ...
        coocaaosapi.getDeviceInfo(function(message) {
            ...
            _czc.push(['_trackEvent', 'D20180821弹窗出现', '酷开商城', TVmodel, '', '']);
...
function nointeresting() {
    _czc.push(['_trackEvent', 'D20180821点击‘不感兴趣’次数', '酷开商城', TVmodel, '', '']);
...
function experienceonclick(obj) {
    if (!gologFlag) {
        _czc.push(['_trackEvent', 'D20180821点击‘去看看’次数', '酷开商城', TVmodel, '', '']);
        pkgButtonLog("去看看","20180405");
    }
    var apkName = obj.getAttribute("apkName");
    coocaaosapi.startAppShopVideo("16323", "http://play.duolebo.com:80/jumpurl/f0b01d7c-8562-4210-b810-3ae3e283c019.m3u8", "（七夕）松下电子智能马桶盖 赠电烤箱", 
    // coocaaosapi.startAppShopZone2("106",
    function(message) {
        console.log(message);
        console.log("启动成功");
        _czc.push(['_trackEvent', 'D20180821启动商城', '酷开商城', TVmodel, '', '']);
        exit();
    }, function(error) {
        console.log("启动失败");
        coocaaosapi.startOrCreateDownloadTask2(
            ...
            function(message) {
                console.log(message);
                console.log("调用成功");
                _czc.push(['_trackEvent', 'D20180821启动下载', '酷开商城', TVmodel, '', '']);
                // exit();
                $("#goToDown").hide();
                $("#walk").hide();
                $("#progress").show();
