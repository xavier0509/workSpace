...
var app = {
    ...
    handleBackButton: function() {
        _czc.push(['_trackEvent', 'D20180808点击‘返回键’次数', '酷开商城', TVmodel, '', '']);
        exit();
    },
    ...
    onDeviceReady: function() {
        ...
        coocaaosapi.getDeviceInfo(function(message) {
            ...
            _czc.push(['_trackEvent', 'D20180808弹窗出现', '酷开商城', TVmodel, '', '']);
    ...
    triggleButton: function() {
        ...
        coocaaosapi.addAppTaskListener(function(message){
             ...
             if(message.status == "ON_REMOVED" && message.url == "http://apk.sky.fs.skysrt.com/uploads/20180206/20180206103450797932.apk" && showa >= 100){
                gologFlag = true;
                $("#deviceready").trigger('itemClick');
             }
...
function nointeresting() {
    _czc.push(['_trackEvent', 'D20180808点击‘不感兴趣’次数', '酷开商城', TVmodel, '', '']);
    pkgButtonLog("不感兴趣","D20180808");
    exit();
}
...
function experienceonclick(obj) {
    if (!gologFlag) {
        _czc.push(['_trackEvent', 'D20180808点击‘确定键’次数', '酷开商城', TVmodel, '', '']);
        pkgButtonLog("点击确定键","D20180808");
    }
    var apkName = obj.getAttribute("apkName");
    coocaaosapi.startAppShopVideo(
        "7337", 
        "http://all.vod.17ugo.com/vod/kukai/U-酷开/厨具/20180711厨具/556841-赛普瑞斯先生黑骑士不粘锅家庭抢购组新U/u.m3u8",
         "赛普瑞斯.先生黑骑士不粘锅家庭抢购组（新）",
    // coocaaosapi.startAppShopZone2("99",
    function(message) {
        console.log(message);
        console.log("启动成功");
        _czc.push(['_trackEvent', 'D20180808启动商城', '酷开商城', TVmodel, '', '']);
        exit();
    }, function(error) {
        console.log("启动失败");
        coocaaosapi.startOrCreateDownloadTask2(
            ...
            function(message) {
                console.log(message);
                console.log("调用成功");
                _czc.push(['_trackEvent', 'D20180808启动下载', '酷开商城', TVmodel, '', '']);
                // exit();
                $("#goToDown").hide();
                $("#walk").hide();
                $("#progress").show();
