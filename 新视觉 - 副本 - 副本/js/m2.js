...
var app = {
    ...
    handleBackButton: function() {
        _czc.push(['_trackEvent', 'D20180810点击‘返回键’次数', '酷开商城', TVmodel, '', '']);
        exit();
    },
    ...
    onDeviceReady: function() {
        ...
        coocaaosapi.getDeviceInfo(function(message) {
            ...
            _czc.push(['_trackEvent', 'D20180810弹窗出现', '酷开商城', TVmodel, '', '']);
...
function nointeresting() {
    _czc.push(['_trackEvent', 'D20180810点击‘不感兴趣’次数', '酷开商城', TVmodel, '', '']);
    pkgButtonLog("不感兴趣","D20180810");
    exit();
}
...
function experienceonclick(obj) {
    if (!gologFlag) {
        _czc.push(['_trackEvent', 'D20180810点击‘确定键’次数', '酷开商城', TVmodel, '', '']);
        pkgButtonLog("点击确定键","D20180810");
    }
    var apkName = obj.getAttribute("apkName");
    // coocaaosapi.startAppShopVideo(
    //     "16147", 
    //     "https://ks3-cn-beijing.ksyun.com/hmsott/960101.mp4-1280x720-edfd4d41.m3u8",
    //      "好买手-华夏保险常青藤医疗组合保证计划",
    coocaaosapi.startAppShopZone2("101",
    function(message) {
        console.log(message);
        console.log("启动成功");
        _czc.push(['_trackEvent', 'D20180810启动商城', '酷开商城', TVmodel, '', '']);
        exit();
    }, function(error) {
        console.log("启动失败");
        coocaaosapi.startOrCreateDownloadTask2(
            ...
            function(message) {
                console.log(message);
                console.log("调用成功");
                _czc.push(['_trackEvent', 'D20180810启动下载', '酷开商城', TVmodel, '', '']);
                // exit();
                $("#goToDown").hide();
                $("#walk").hide();
                $("#progress").show();
